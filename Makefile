...
MATITA_LIB=$(notdir $(basename $(wildcard $(MATITA_IN)/*.dk)))
...
MATITA_FILES=$(MATITA_LIB:%=$(MATITA_OUT)/%.dk)
...
$(MATITA_FILES): $(MATITA_OUT)/%.dk: $(MATITA_IN)/%.dk
	$(UNIVERSO) $(OPTIONS) --elab-only -o $@ $<
.PHONY: debug
debug:
	@echo $(MATITA_FILES)
...
.PHONY: dklevel
dklevel: bin
	~/Git/dklevel/_build/install/default/bin/dklevel -v $(FILES_LIGHT)
