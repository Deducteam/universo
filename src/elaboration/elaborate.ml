...
let rec mk_term : t -> T.term -> T.term = fun env ->
  fun t ->
    if Var.is_pre_var t then
      ...
      | T.Lam (lc,id,None,te) ->
        T.mk_Lam lc id None (mk_term env te)
      | T.Pi (lc,id,tya,tyb) ->
...
let mk_term : t -> T.term -> T.term = fun env t ->
...
let mk_rule : t -> 'a R.rule -> 'a R.rule = fun env rule -> R.(
...
let mk_entry : t -> E.entry -> E.entry = fun env e ->
