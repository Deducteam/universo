...
let variables = Hashtbl.create 257
...
let add_constraint_univ var univ =
  ...
  let level = Arithmetic.Integer.mk_numeral_i ctx (univ+1)
  in
...
let rec univ_of_int n =
  if n = 0 then
    Term.mk_Const Basic.dloc Cic.z
  else
    Term.mk_App (Term.mk_Const Basic.dloc Cic.s) (univ_of_int (n-1)) []

let univ_of_int n =
  if n = 0 then
    Term.mk_Const Basic.dloc Cic.prop
  else
    Term.mk_App (Term.mk_Const Basic.dloc Cic.typ) (univ_of_int (n-1)) []
